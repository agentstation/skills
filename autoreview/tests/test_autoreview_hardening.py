...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_review_prompt_applies_requested_priority_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            prompt = self.helper["render_review_prompt"](
                repo,
        ...
        threshold_prompt = self.helper["apply_finding_threshold_prompt"](
            argparse.Namespace(max_priority="P2"),
            "Additional review context.",
        )
        self.assertIn("Finding threshold: report only P0, P1, P2.", threshold_prompt)
        self.assertIn("Omit all lower-priority observations", threshold_prompt)
        self.assertTrue(threshold_prompt.endswith("Additional review context."))
    ...
    def test_pre_pr_reuses_clean_attestation_after_docs_only_commit(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = init_repo(root)
            ...
            git(repo, "commit", "-qm", "change code")
            ...
            self.assertIn("clean attestation stored", first.stdout)

            readme.write_text("review proof\n", encoding="utf-8")
            git(repo, "add", "README.md")
            git(repo, "commit", "-qm", "record review proof")
            record_path.unlink()
            ...
            self.assertIn("clean attestation reused", second.stdout)
            self.assertFalse(record_path.exists())

            forced = subprocess.run(
                [*command, "--no-review-cache"],
                cwd=repo,
                env=env,
                text=True,
                capture_output=True,
            ...
            self.assertNotIn("clean attestation reused", forced.stdout)
            self.assertTrue(record_path.is_file())
    ...
    def test_resolve_engine_binary_supports_local_isolated_adapters(self) -> None:
        resolve_engine_binary = self.helper["resolve_engine_binary"]
        reviewers = (
            argparse.Namespace(engine="opencode", opencode_bin="opencode"),
            argparse.Namespace(
                engine="cursor",
        ...
        with mock.patch.dict(
            resolve_engine_binary.__globals__,
            {
                "find_command": lambda name, _repo: f"/trusted/{name}",
                "ensure_cursor_supported": lambda *_args: "--print --output-format",
                "safe_engine_env": lambda *_args, **_kwargs: {},
            },
        ):
